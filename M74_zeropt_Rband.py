...
im = fits.open(path+'M74_wcs_Rband.fits')[0].data
...
for n in range(len(stars)):
    a = pha.CircularAperture((stars[n,0], stars[n,1]), 20)
    apertures.append(a)
    ff = pha.aperture_photometry(im, a)
    flux20.append(ff['aperture_sum'][0])

plt.figure(dpi=150)
plt.imshow(im, cmap='viridis', clim=[0, 300])
...
zp_single = [m_Ha[i] + 2.5 * np.log10(flux20[i]) for i in range(len(stars))]
