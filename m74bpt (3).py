...
with fits.open(filename) as hdul:
   ...
   # imNii = hdul['NII6548_FLUX'].data
    ...
    imOii_err= hdul['OIII5006_FLUX_ERR'].data
...
for i in range(len(sn_array)):
    for j, aperture in enumerate(aperture_array):
        ...
        cutout = mask.to_image(sn_array[i].shape) #adatta la maschera alla dimensione della immagine, in particolare 1 se pixel è coperto un valore intermedio se parzialmente coperto e 0 se no
         ...
         # Pixel coperti dall'apertura
        covered_pixels = sn_array[i][cutout.astype(bool)]
        #if not np.isnan(sn_array[i][cutout.astype(bool)]).all():
        #    valid_aperture_idx.append(j)
        if np.count_nonzero(~np.isnan(covered_pixels)) > min_valid_pixels:
            valid_aperture_idx.append(j)
            rapp_perc = 100*  np.count_nonzero(~np.isnan(covered_pixels))/len(covered_pixels)
            #print(f"i pixel validi sono il {rapp_perc} % del totale")

# Plot dell'immagine con le aperture valide
for i in range(len(sn_array)):
    plt.imshow(sn_array[i], cmap='viridis', clim=[0, 1000])
    for j in valid_aperture_idx:
        aperture_array[j].plot(color='red', lw=0.5)
    plt.colorbar()
    plt.show()
aperture_array = [aperture_array[i] for i in valid_aperture_idx]

print(len(valid_aperture_idx))
...
def kewley(ratio_x):
  return 1.3+ 0.72/(ratio_x -0.32) # le lambda non sono proprio le stesse
kew_law = []
x = np.linspace(-1.2, 0.02, len(ratio_x))
for i in range(len(ratio_x)):
  kew_law.append(kewley(x[i]))
plt.scatter(np.log10(ratio_x), np.log10(ratio_y), s= 10)
plt.plot(x, kew_law, color = 'red')
plt.text(-1, 0.25, "HII")
plt.text(-0.6, 0.75, "Seyfert")
plt.text(-0.1, 0 , "LINER")
